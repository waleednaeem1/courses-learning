...
class CoursesController extends Controller
{
    ...
    public function coursesList()
    {
       ...
       $userCartItemsCount = count(CartItem::where([['customer_id', $user->id], ['type' , 'course']])->get());
        $data['userCartItemsCount']=$userCartItemsCount;
    ...
    public function applyCoach($courseSlug, $coachId ,$user_id){
        ...
        $shallowCourse = ShallowCourse::where(['slug'=> $courseSlug, 'purchased_user_id'=> $user_id, 'coach_id'=> $coachId])->first();
          if(!isset($shallowCourse->coach_id)){
                ShallowCourse::where(['slug'=> $courseSlug, 'purchased_user_id'=> $user_id])->update(['coach_id' => $coachId]);
                $message = 'Coach applied successfully. You can safely close this page.';
            }
            elseif($shallowCourse->coach_id == $coachId){
                $message = 'Coach already applied. You can safely close this page.';
            }
            elseif($shallowCourse->coach_id != $coachId){
                ShallowCourse::where(['slug'=> $courseSlug, 'purchased_user_id'=> $user_id])->update(['coach_id' => $coachId]);
                $message = 'Coach updated applied successfully. You can safely close this page.';
            }
            else{
               $message = 'Coach did not applied.';
            }

           return view('auth.verified_coach_access',compact('message'));
    }
    ...
    public function completeDetail($course_slug)
    {
        // die($course_slug);
        die("Working on it. It will live soon!");
    }
    ...
    public function courseDetail($course_slug)
    {
        ...
        if(isset($data['course']) && $data['course'] != ''){
            ...
            // course completion progress calculate
             $result =getCourseProgress($data['course']->id);
             $is_course_complete=$result[0];
             $percentage=$result[1];
    ...
    public function courseModuleDetail($course_slug, $module_slug)
    {
        $user_id = auth()->user()->id;
    ...
    public function courseModuleSectionDetail($course_slug, $module_slug,$section_slug)
    {
        $user_id = auth()->user()->id;
    ...
    public function courseModuleSectionExcerciseDetail( $course_slug, $module_slug,$section_slug,$exercise_slug,$parent_course_module_section_id='')
    {
        $user_id = auth()->user()->id;
        ...
        $data['page_type'] = "course_module_section_exercise_details";
         ...
         $totalExerciseComplete= ExerciseCompletion::where(['section_id'=>$data['section']->id,'user_id'=>auth()->user()->id,'exercise_id'=>$data['exercise']->id])->count();

          $exerciseComplete=0;
          if($totalExerciseComplete==1){
          $exerciseComplete=1;
          }

         $data['exerciseComplete'] = $exerciseComplete;
